...
import { Upload, FileBox, X, ArrowRight, Loader2 } from "lucide-react";
import { toast } from "sonner";

const materials = [
  { id: "pla", name: "PLA", desc: "Everyday prototypes", multiplier: 1.0 },
  ...
  { id: "carbon", name: "Carbon Fiber", desc: "Aerospace strength", multiplier: 3.4 },
];
...
const QuoteBuilder = () => {
  ...
  const acceptFile = useCallback((f: File | undefined | null) => {
    ...
    if (!okExt) {
      toast.error("Unsupported file", { description: "Use STL, OBJ, STEP, or 3MF." });
      return;
    }
    if (!okSize) {
      toast.error("File too large", { description: "Max 200MB." });
      return;
    }
    setFile(f);
  ...
  const submit = async () => {
    ...
    toast.success("Quote sent!", {
      description: `Total $${price.total.toFixed(2)} · we'll email you within 5 minutes.`,
    });
  };
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-card p-6 md:p-12">
          ...
          <div className="relative z-10 grid lg:grid-cols-5 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-3 space-y-8">
              <div>
                <p className="text-sm font-medium text-primary uppercase tracking-wider mb-3">Instant quote</p>
                <h2 className="font-display text-3xl md:text-5xl font-semibold tracking-tight text-gradient">
                  Build your print.
                </h2>
                <p className="text-muted-foreground mt-3">Drop a file, pick a material, get a price. No account required.</p>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="h-11 w-11 rounded-xl border border-border bg-background/40 hover:border-primary/40 transition-colors"
                  >−</button>
                  <input
                    type="number"
                    min={1}
            ...
            <div className="lg:col-span-2">
              <div className="lg:sticky lg:top-28 rounded-2xl border border-primary/30 bg-background/60 backdrop-blur-xl p-6 shadow-elegant">
                ...
                <p className="text-xs text-muted-foreground mt-3 text-center">
                  Final pricing confirmed after model analysis.
                </p>
              </div>
            </div>
          </div>
